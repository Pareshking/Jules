        ...
        try:
            ...
            if results.empty:
                ...
                cols_to_show = [
                    ...
                    'Rank Velocity', 'Rank 1M Ago', 'Rank 2M Ago', 'Rank 3M Ago'
                ]

                # Ensure columns exist
                cols_to_show = [c for c in cols_to_show if c in results.columns]

                def style_rank_velocity(val):
                    if pd.isna(val):
                        return ''
                    if val > 0:
                        return 'color: green'
                ...
                st.dataframe(
                    results[cols_to_show].style
                    .format(format_mapping, na_rep="")
                    .map(style_rank_velocity, subset=['Rank Velocity'] if 'Rank Velocity' in results.columns else []),
