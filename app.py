...
def color_velocity(val):
    """
    Color rank velocity: Green for positive, Red for negative, Black for zero.
    """
    if pd.isna(val):
        return ''
    if val > 0:
        ...
        try:
            ...
            if results.empty:
                ...
                st.dataframe(
                    ...
                    .map(color_velocity, subset=['Rank Velocity'] if 'Rank Velocity' in results.columns else None),
