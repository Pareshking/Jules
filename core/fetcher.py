    ...
    for index_name in indices_names:
        ...
        try:
            ...
            if symbol_col:
                ...
                all_symbols.update([s.strip() for s in symbols if not s.strip().startswith('DUMMY')])
            else:
...
def fetch_price_data(tickers: List[str], period: str = "3y") -> pd.DataFrame:
    ...
    if isinstance(data.columns, pd.MultiIndex):
        # We want to select the 'Close' price and flatten the multi-index.
        # This gets the 'Close' prices for all tickers
    ...
    if close_data.index.tzinfo is not None:
