...
# Allow individual URL overrides via env vars like NIFTY_50_URL
INDICES_URLS = {
...
# Can be overridden by env var MOMENTUM_WINDOWS (json list)
_windows_env = os.environ.get("MOMENTUM_WINDOWS")
if _windows_env:
    try:
        MOMENTUM_WINDOWS = json.loads(_windows_env)
    except json.JSONDecodeError:
        print("Warning: Failed to parse MOMENTUM_WINDOWS env var. Using default.")
        MOMENTUM_WINDOWS = [21, 63, 126, 189, 252]
else:
    MOMENTUM_WINDOWS = [21, 63, 126, 189, 252]

# Weights corresponding to the windows
# Can be overridden by env var MOMENTUM_WEIGHTS (json list)
_weights_env = os.environ.get("MOMENTUM_WEIGHTS")
if _weights_env:
    try:
        MOMENTUM_WEIGHTS = json.loads(_weights_env)
    except json.JSONDecodeError:
        print("Warning: Failed to parse MOMENTUM_WEIGHTS env var. Using default.")
